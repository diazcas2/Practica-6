...
Datos<-read_delim('data/NosAutoanalizamos_TD2019 - Hoja 1 - Clean.tsv',delim='\t',escape_double = FALSE,trim_ws=TRUE) %>% mutate(across(ends_with('cat'),as.factor))
...
Datos %>% pivot_longer(cols =ends_with('.num') ,names_to ='asignatura' , values_to = 'valor') %>% group_by(asignatura) %>% summarise(mean=mean(valor,na.rm=TRUE),sd=sd(valor,na.rm=TRUE),median=median(valor,na.rm=TRUE),iqr=IQR(valor,na.rm=TRUE))
Datos
```
...
Datos %>%pivot_longer(cols =ends_with('.num') ,names_to ='asignatura' , values_to = 'valor') %>%  filter(asignatura=='ALG.num') %>% ggplot(aes(valor))+geom_histogram(aes(y=stat(density)))+geom_density()+geom_vline(aes(xintercept=mean(valor)),color='red',size=1)
...
library(car)
valores<-Datos %>%pivot_longer(cols =ends_with('.num') ,names_to ='asignatura' , values_to = 'valor') %>%  filter(asignatura=='ANM.num')
qqPlot(valores$valor)
```
...
mosaicplot(table(Datos$ALG.cat, Datos$ANM.cat))
```
...
Datos %>% select(ends_with('.num')) %>% cor(use='complete.obs',method='pearson')
Datos %>% select(ends_with('.num')) %>% cor(use='complete.obs',method='spearman')
```
...
sqrt(Datos$ALG.num)
Datos %>% mutate(raiz.num=sqrt(Datos$ALG.num)) %>% select(ends_with('.num')) %>% cor(use = "complete.obs", method = "pearson")

Datos %>% mutate(raiz.num=sqrt(Datos$ALG.num)) %>% select(ends_with('.num')) %>% cor(use = "complete.obs", method = "spearman")
...
ggpairs(Datos %>% select(ends_with('.num')))
```
...
#Rechazamos que el tipo de calificación es independiente
```


```{r Solucion_Ej9b, include=TRUE, echo=TRUE, eval=TRUE, results=TRUE}
chisq.test(Datos$ALG.cat,Datos$Sex,correct=FALSE)
#Existe suficiente evidencia estadística para aceptar la hipótesis nula, es decir, hay independencia
```
